...
class EvidenceBundleStore:
    ...
    def write_source_snapshots(self, candidates: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        snapshots = [self.write_source_snapshot(candidate) for candidate in candidates]
        index = {
            "bundle_count": len(snapshots),
            "bundles": [self._index_bundle(snapshot) for snapshot in snapshots],
        }
        write_json_object(self.data_folder / "evidence_bundle_index.json", index)
        return index

    def write_source_snapshot(self, candidate: dict[str, Any]) -> dict[str, Any]:
        metadata_path = self.source_metadata_path(candidate)
        write_json_object(metadata_path, candidate)
        ...
        snapshot = {
            **self._snapshot_identity(candidate),
            "source_metadata": self._available_metadata_record(metadata_path),
            "source_video": self.source_video_state(download_status, video_path),
            "artifacts": {},
        }
        write_json_object(self.snapshot_path(candidate), snapshot)
        return snapshot
    ...
    def load_snapshot(self, candidate: dict[str, Any]) -> dict[str, Any]:
        ...
        if snapshot is None:
            return self._missing_snapshot(candidate)
        return snapshot

    def _snapshot_identity(self, candidate: dict[str, Any]) -> dict[str, Any]:
        return {
    ...
    def _available_metadata_record(self, metadata_path: Path) -> dict[str, Any]:
        return {
            "state": "available",
            "path": relative_path(metadata_path, self.run_folder),
        }

    def _missing_snapshot(self, candidate: dict[str, Any]) -> dict[str, Any]:
        return {
            **self._snapshot_identity(candidate),
            "source_metadata": {
                "state": "missing",
    ...
    def _index_bundle(self, snapshot: dict[str, Any]) -> dict[str, Any]:
        return {
