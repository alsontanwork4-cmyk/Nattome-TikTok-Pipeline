...
class DashboardFastAPISettingsTest(unittest.TestCase):
    ...
    def test_run_detail_only_renders_run_workbench(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            data_client = FakeDashboardDataClient()
            client, _ = self._client(Path(temp_dir), data_client)
            ...
            self.assertEqual(response.status_code, 200)
            self.assertIn("Gut health hook", response.text)
            self.assertIn("TikTok Scraper Run", response.text)
            self.assertEqual(response.text.count('method="post"'), 1)
