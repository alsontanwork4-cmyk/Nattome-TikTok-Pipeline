...
class PipelineHealthTest(unittest.TestCase):
    def test_completed_run_reports_plain_language_operational_health(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("Source video snapshots are ready", summary.impact_summary)
            self.assertEqual(
    ...
    def test_partial_run_warns_when_some_artifacts_are_missing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self._write_fixture_workspace(
                ...
                phases=[
                    {"name": "candidate_selection", "status": "completed"},
                    {"name": "source_video_snapshots", "status": "partial"},
                ],
    ...
    def test_missing_source_video_snapshots_block_review(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            self._write_fixture_workspace(
                workspace,
                ...
                phases=[
                    {"name": "candidate_selection", "status": "completed"},
                    {
                        "name": "source_video_snapshots",
                        "status": "failed",
                        "exception": "Source video download failed",
                    },
                ],
            ...
            source_videos = self._item(summary, "source_videos")
            self.assertEqual(source_videos.severity, "blocked")
            self.assertIn("No selected source videos", source_videos.impact)
    ...
    def test_pipeline_health_does_not_change_scrape_quality_score(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self._write_fixture_workspace(
                ...
                phases=[
                    {"name": "candidate_selection", "status": "completed"},
                    {"name": "source_video_snapshots", "status": "failed", "exception": "download"},
                ],
                bundle_state="missing",
                with_source_video=True,
            )
    ...
    def _write_fixture_workspace(
        ...
        *,
        ...
        for folder in [raw_scrapes, data_folder, evidence_folder]:
