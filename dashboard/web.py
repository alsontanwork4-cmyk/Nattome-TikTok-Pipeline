...
from .manual_runs import list_manual_runs, trigger_manual_run
...
def render_page(active_path: str, workspace: Path) -> str:
    ...
    if active_path == "/":
        ...
        overview = _render_run_history(workspace)
    else:
...
def _render_run_history(workspace: Path) -> str:
    manual_runs = list_manual_runs(workspace)
    return f"""
      <h1>Run History</h1>
      <p class="lede">Manual dashboard runs are recorded separately from scheduled Batch Analysis Runs.</p>
      {_render_manual_run_controls()}
      <section class="panel wide-panel" aria-label="Manual run status">
        <h2>Manual Run Status</h2>
        {_render_manual_run_list(manual_runs)}
      </section>
    """
...
def _render_manual_run_list(manual_runs: list[object]) -> str:
    if not manual_runs:
        return '<p class="muted">No manual dashboard runs have been triggered yet.</p>'
    items = "\n".join(_render_manual_run_item(run) for run in manual_runs)
    return f'<ol class="history-list">{items}</ol>'


def _render_manual_run_item(run: object) -> str:
    output_items = "\n".join(
        f"<li>{html.escape(label.replace('_', ' ').title())}: <code>{html.escape(path)}</code></li>"
        for label, path in getattr(run, "output_paths").items()
    )
    error = getattr(run, "error_text")
    error_markup = f'<p class="muted">Error: {html.escape(error)}</p>' if error else ""
    return f"""
      <li class="history-item">
        <p><strong>{html.escape(getattr(run, "run_type").replace("_", " ").title())}</strong> - {html.escape(getattr(run, "status"))}</p>
        <p class="muted">Triggered {html.escape(getattr(run, "triggered_at"))} by {html.escape(getattr(run, "triggered_by"))} using {html.escape(getattr(run, "config_version"))}. Source: {html.escape(getattr(run, "source_type"))}.</p>
        {error_markup}
        <ul class="compact-list">{output_items}</ul>
      </li>
    """
